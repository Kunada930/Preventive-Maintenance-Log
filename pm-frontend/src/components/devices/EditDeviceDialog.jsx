...
export default function EditDeviceDialog({
  ...
  useEffect(() => {
    if (device && open) {
      const purchaseDate = device.datePurchased
        ? new Date(device.datePurchased)
        : null;
      ...
      setFormData({
        ...
        datePurchased: device.datePurchased || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        showAlert("Success", "Device updated successfully", "default");
        onSuccess?.();
        setTimeout(() => {
          onOpenChange(false);
        }, 1500);
